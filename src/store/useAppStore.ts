...
import { INITIAL_USERS, INITIAL_PRODUCTS, INITIAL_SUPPLIERS, INITIAL_CASH_SESSION } from '../shared/mockData';
...
interface AppState {
  ...
  completeSale: (paymentMethod: PaymentMethod, cashReceived?: number) => Sale | null;
}
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  addToCart: (product) => {
    ...
    if (existingIndex > -1) {
      ...
      if (existing.quantity >= product.stock) return; // Stock limit protection
  ...
  initiateQRPayment: (saleTotal) => {
    ...
    // Mercado Pago EMV Co standard style payload simulation
    const mpPayload = `00020101021243500016com.mercadopago023000000123456789520459995303032540${saleTotal.toFixed(
      2
    )}5802AR5915NEXPOS_COMMERCE6012BUENOS_AIRES62150511${saleCode}6304`;
  ...
  confirmQRPaymentSuccess: (qrTxId) => {
